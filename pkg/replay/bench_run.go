...
func createScratchAccountsDb(dir string) (*accountsdb.AccountsDb, error) {
	...
	// OpenDb requires the largest_file_id marker (8 bytes LE); a fresh db
	// starts at zero.
	var largestFileId [8]byte
	binary.LittleEndian.PutUint64(largestFileId[:], 0)
	if err := os.WriteFile(filepath.Join(dir, "largest_file_id"), largestFileId[:], 0644); err != nil {
		return nil, err
	}
...
func benchConfigureInitialBlock(acctsDb *accountsdb.AccountsDb,
	...
	// Restore the Clock sysvar as of the last replayed slot, mirroring
	// configureInitialBlockFromResume: the first bundled slot must use the
	// parent's clock rather than whatever value the bundled account carries.
	if len(resumeState.Clock) > 0 {
		clockAcct, err := acctsDb.GetAccount(block.Slot, sealevel.SysvarClockAddr)
		if err != nil {
			return fmt.Errorf("loading clock sysvar account: %w", err)
		}
		...
		if err := clock.UnmarshalWithDecoder(bin.NewBinDecoder(clockAcct.Data)); err != nil {
			return fmt.Errorf("decoding restored clock sysvar: %w", err)
		}
...
func RunBenchBundle(opts BenchRunOpts) (*BenchRunResult, error) {
	...
	pt := &persistedTracker{}
	// Fresh from-genesis coverage: bundles hold finalized mainnet blocks, so
	// no transaction can duplicate a pre-window one; within-window dedup is
	// maintained by ProcessBlock recording into the cache, and any behavioral
	// divergence is still caught by the recorded TxMetas.
	...
	for _, path := range blockPaths {
		...
		start := time.Now()
		// tail=nil (rooted-durable off: the scratch db is written directly) and
		// alpenglowClock=false (bundles are TowerBFT mainnet blocks).
		lastSlotCtx, err = ProcessBlock(acctsDb, block, epochSchedule, opts.TxParallelism, dbgOpts, pt, nil, transactionStatuses, false)
		if err != nil {
