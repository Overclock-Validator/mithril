...
type BenchExportOpts struct {
	// AccountsPaths are the accountsdb shard mounts, primary first
	// (same as the node's storage.accounts config).
...
func ExportBenchBundle(opts BenchExportOpts) error {
	if len(opts.AccountsPaths) == 0 {
		return fmt.Errorf("no accounts paths given")
	}
	if opts.NumBlocks == 0 {
		return fmt.Errorf("num blocks must be > 0")
	}

	mithrilState, err := state.LoadState(opts.AccountsPaths[0])
	if err != nil {
	...
	acctsDb, err := accountsdb.OpenDb(opts.AccountsPaths)
	if err != nil {
	...
	stateData, err := os.ReadFile(filepath.Join(opts.AccountsPaths[0], "mithril_state.json"))
	if err != nil {
		return err
	}
