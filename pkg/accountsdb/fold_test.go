...
func TestAccountResolverRegistrationDoesNotClearNewerResolver(t *testing.T) {
	db := &AccountsDb{RootedDurable: true}
	key := solana.PublicKey{1}
	cleanupFirst := db.InstallAccountResolver(func(uint64, solana.PublicKey) (*accounts.Account, error) {
		return &accounts.Account{Key: key, Lamports: 1}, nil
	})
	cleanupSecond := db.InstallAccountResolver(func(uint64, solana.PublicKey) (*accounts.Account, error) {
		return &accounts.Account{Key: key, Lamports: 2}, nil
	})
	defer cleanupSecond()

	cleanupFirst()
	acct, err := db.GetAccount(9, key)
	require.NoError(t, err)
	require.Equal(t, uint64(2), acct.Lamports)
}

func TestRootedDurableRejectsDirectStoreAccounts(t *testing.T) {
	db := &AccountsDb{RootedDurable: true}
	err := db.StoreAccounts([]*accounts.Account{{Key: solana.PublicKey{1}}}, 9, nil)
	require.ErrorIs(t, err, ErrRootedDirectWrite)
}
