...
The generated config has production-oriented defaults - you only need to
customize the storage paths for your setup. Host-adaptive tuning remains
omitted so the runtime can size it from GOMAXPROCS.
...
# Core operational options are listed below; host-adaptive settings remain
# omitted so the runtime can size them. See config.example.toml for the full
# reference and advanced tuning.
...
[storage.compact]
enabled = true                  # Default-on bounded appendvec reclamation
enforce_disk_reserve = true     # Fail closed before ENOSPC; false is an unsafe/unbounded escape hatch
interval_seconds = 30           # Routine pressure check cadence
min_free_mb = 0                 # 0 = filesystem-adaptive hard reserve
target_free_mb = 0              # 0 = filesystem-adaptive maintenance target
min_dead_fraction = 0.7         # Routine write-amplification threshold
emergency_min_dead_fraction = 0.2 # Lower threshold only under hard disk pressure
max_move_mb = 64                # Soft routine-cycle live-byte target
max_scan_mb = 256               # Soft routine-cycle scan target
max_source_mb = 64              # Hard routine pre-scan cap; emergency compaction removes it

# ── Network ──────────────────────────────────────────────────────────────
[network]
cluster = "alpenglow"     # Required by this branch's V2 AccountsDB runtime; use dev for classic clusters
...
account_index_shards = 1024             # Durable V2 routing; changing an existing DB requires a fresh snapshot.
account_index_max_hot_keys = 2097152    # Exact mutable-overlay key ceiling.
account_index_max_hot_mb = 512          # Conservative accounted mutable-overlay RAM ceiling.
account_index_max_checkpoint_selected_mb = 1024 # Root-selected exact checkpoint artifact ceiling.
account_index_max_checkpoint_physical_mb = 2048 # Includes builds and reader-pinned obsolete generations.
account_index_seal_keys = 8192          # Per-shard exact checkpoint threshold.
account_index_seal_max_age_ms = 30000   # Avoid tiny checkpoints on mostly-idle shards.
account_index_rebase_keys = 65536       # Roll one shard's exact delta into its immutable base.
account_index_journal_rewrite_mb = 512  # Bounded-state WAL rewrite threshold.
# Checkpoint workers and concurrent seals are intentionally omitted so the
# runtime adapts them to this host (GOMAXPROCS workers, at most 8 seals).
# Set account_index_checkpoint_workers/account_index_max_concurrent_seals only
# after host-specific measurement.
account_index_rebase_workers = 1        # Required: shared extent lineage currently serializes rebases.
working_set_max_mb = 1024               # Hard account-layer RAM limit; a slot that would exceed it is rejected atomically.
